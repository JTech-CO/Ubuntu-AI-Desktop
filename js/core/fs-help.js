...
재현한 에뮬레이터입니다.
  ...
  * 서버가 없습니다. 모든 계산은 여러분의 브라우저 탭 안에서
    일어납니다. Gemini API를 부를 때와, python3 를 처음 실행할 때
    Python 런타임(Pyodide)을 내려받을 때를 제외하면 네트워크 요청이
    나가지 않습니다.
...
출력합니다. 아래가 그 경계선입니다.
  ...
  가상 파일시스템
      ...
      모든 앱이 이 하나를 공유하고 IndexedDB에 저장됩니다.
  ...
  진짜 Python 3.12
      python3 는 흉내가 아니라 WebAssembly로 컴파일된 진짜 CPython
      3.12.7(Pyodide 0.27.7)입니다. 처음 실행할 때 cdn.jsdelivr.net 에서
      약 6 MB를 내려받고, 그 뒤로는 브라우저 캐시에서 불러옵니다.
      프로그램은 홈, /tmp, 지금 디렉터리의 파일을 그대로 보고, 만들거나
      고치거나 지운 파일은 데스크톱 파일시스템에 그대로 반영됩니다.
      input() 은 터미널에서 입력을 받습니다.
  ...
  네트워크 명령은 모사입니다
      ...
      페이지 밖으로 나가는 통신은 Gemini API 호출과, python3 를 처음
      쓸 때 Pyodide 를 내려받는 것뿐입니다.

  Code - OSS의 Run은 일부만 AI 해석입니다
      셸 스크립트와 Python 은 진짜로 실행됩니다. Python 파일은 저장한
      뒤 통합 터미널에서 python3 로 실행되므로 input() 도 됩니다.
      JavaScript·C·C++·Java 는 Gemini에게 보내 결과를 예측하게 하고,
      출력 패널에 "AI가 시뮬레이션한 결과"라고 분명히 표시합니다.
      API 키가 없으면 결과를 지어내는 대신 없다고 말합니다.
...
──────────────────────────────────────────────────────────────────
  ...
  Alt + F2 — 명령 실행 대화상자
      ...
      열린 창, 프로세스 표, 파일시스템 통계, 저장 위치와 사용량을
...
외부 명령 190개와 셸 빌트인 24개, 모두 214개가 등록되어 있습니다.
별칭까지 세면 터미널이 아는 이름은 229개입니다.
살아 있는 목록은 help, 자세한 설명은 man <명령어> 로 볼 수 있습니다.
  ...
  Python (1)
  ──────────
  python3             진짜 CPython 3.12.7 (Pyodide). 별칭: python3.12
                        python3              대화형 인터프리터 (Ctrl+D 로 종료)
                        python3 hello.py     스크립트 실행
                        python3 -c '코드'    한 줄 실행
                        python3 -m json.tool 모듈 실행
                        ./hello.py           #!/usr/bin/env python3 줄로 실행
                        echo 5 | python3 -c 'print(int(input())*2)'
                      표준 라이브러리만 있습니다(numpy 같은 패키지는 새 우분투처럼
                      설치되어 있지 않음). python 이나 pip 를 치면 우분투와
                      같은 command-not-found 안내가 나옵니다.
                      Ctrl+C 는 인터프리터를 멈추는 방식이라 KeyboardInterrupt 를
                      잡을 수 없고, 대화형 모드의 변수는 사라집니다.
                      소켓, subprocess, os.fork 는 쓸 수 없습니다.
...
──────────────────────────────────────────────────────────────────
  ...
  초기화
  ──────
  파일시스템은 IndexedDB 에, 설정과 API 키는 localStorage 에
  남습니다. 갓 설치한 상태로 되돌리려면 터미널에서 reset 을
  치거나, 브라우저 개발자 콘솔에서 다음을 실행하세요.

      UAD.reset()

  localStorage.clear() 만으로는 더 이상 초기화되지 않습니다.
  파일시스템이 거기 없기 때문입니다.

  저장 공간
  ─────────
  예전에는 파일시스템 전체를 localStorage(출처당 약 5MB)에 넣었는데,
  풀HD 스크린샷 한 장이 데이터 URL로 1~4MB라 두어 장이면 가득
  찼습니다. 게다가 실패해도 아무 말이 없어서, 새로 고치면 마지막
  저장 이후의 작업이 조용히 사라졌습니다.

  지금은 IndexedDB(수 GB)에 저장하고, 16KB 이상인 파일은 따로
  보관해 바뀐 것만 다시 씁니다. 저장에 실패하면 알림이 뜨고,
  다음 저장이 성공할 때까지 남아 있습니다. 휴지통에 든 파일도
  공간을 차지하니, 공간이 모자라면 휴지통까지 비우세요.

  탭 두 개
  ────────
  데스크톱을 두 탭에서 열면 먼저 연 탭만 저장합니다. 나중에 연
  탭은 읽기 전용이 되고, 화면 아래에 "이 탭에서 바꾼 내용은
  저장되지 않습니다"와 [여기서 사용] 버튼이 뜹니다. 누르면 먼저
  연 탭이 변경 사항을 저장한 뒤 넘겨줍니다.
